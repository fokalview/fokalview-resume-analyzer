...
const comparison={score:99,summary:'Original',strengths:['Built SQL reports'],improvements:[],evaluations:[],sourceEvidence:[{claim:'SQL',resumeQuote:'Built SQL reports',jobQuote:'SQL required'},{claim:'Invented',resumeQuote:'Led a team of 50',jobQuote:'SQL required'}]};
async function review(writer={summary:'Edited',strengths:['Built SQL reports'],improvements:[]}) {
 ...
 if(name==='job-extraction')return {requirements:[],jobDetails:{title:'Analyst'},jobQualifications:{requiredSkills:['Python']}};
...
test('separate stages preserve locked qualifications, derive readiness, and discard fabricated quotes',async()=>{
 const {result,stages}=await review();assert.equal(stages.length,4);assert.equal(result.score,null);assert.equal(result.readiness.level,'More information needed');
...
test('writer cannot mutate assessment or profile and malformed output falls back',async()=>{
 const {result}=await review({score:100,summary:'Rewrite',strengths:[],improvements:[]});
 assert.equal(result.score,null);assert.equal(result.readiness.level,'More information needed');assert.equal(result.summary,'Original');assert.ok(result.orchestration.stages.includes('report-writing-fallback'));
});
