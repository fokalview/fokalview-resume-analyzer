...
export default function CandidateDashboard({ analysis, targetRole, userIdentity, onNavigate }: Props) {
  ...
  const previousScore = analysis ? undefined : resumeRecords.slice(1).find(record => record.opportunityId && record.opportunityId === latestRecord?.opportunityId && record.analysis?.scoringVersion === currentAnalysis?.scoringVersion)?.analysis?.score;
  ...
  if(loading) return <section className="screen" role="status">Loading your career dashboardâ€¦</section>;
    ...
    <section className="screen dashboard-screen">
      <div className="dashboard-hero">
        <div>
          ...
          <p>
            Pick up your latest review or compare your resume with a new job.
          </p>
      ...
      <section className="dashboard-grid">
        ...
        <article className="dashboard-panel activity-panel">
          ...
          <div className="mini-metrics">
            <Metric label="Saved opportunities" value={applications.length} />
            <Metric label="Interviews" value={stats.interviews} />
            <Metric label="Offers" value={stats.offers} />
            <Metric label="In interview stages" value={`${stats.responseRate}%`} />
          </div>
...
function applicationStatusCounts(applications: ApplicationRecord[]) {
  ...
  const counts = applications.reduce<Record<string, number>>((nextCounts, item) => {
    const normalized = opportunityStage(item.status);
...
function buildNextSteps(analysis: ResumeAnalysis | null, applications: ApplicationRecord[]) {
  ...
  if (applications.length && !applications.some((item) => opportunityStage(item.status) === "Interviewing")) {
...
function applicationStats(applications: ApplicationRecord[]) {
  const interviews = applications.filter((item) => opportunityStage(item.status) === "Interviewing").length;
  const offers = applications.filter((item) => opportunityStage(item.status) === "Offer").length;
  return {
    interviews,
    offers,
