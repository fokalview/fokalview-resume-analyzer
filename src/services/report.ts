...
<html>
  ...
  <body>
    ...
    ${currentAnalysis ? `<div class="score">${currentAnalysis.score}% career readiness</div><p>${escapeHtml(currentAnalysis.summary)}</p><p class="method">Scoring method: ${escapeHtml(currentAnalysis.scoringVersion || "SagittaIQ fixed readiness rubric")}. Scores are repeatable for the same resume text, extracted requirements, and scoring version. They do not predict hiring decisions.</p>${currentAnalysis.scoreAudit ? `<p class="method"><strong>Independent score audit:</strong> ${escapeHtml(currentAnalysis.scoreAudit.verdict)}. ${escapeHtml(currentAnalysis.scoreAudit.explanation)} Expected range: ${currentAnalysis.scoreAudit.expectedMin}-${currentAnalysis.scoreAudit.expectedMax}.</p>` : ""}` : ""}
    ${reportSections}
    ${applications.length ? `
